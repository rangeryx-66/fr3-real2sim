"""Seal the grasp-family implementation and the frozen 90-episode protocol."""
import hashlib
import json
...
NAMES = [
    ...
    "FAMILY_FORMAL_PROTOCOL.json", "FAMILY_ASSET_HASHES.json",
    "config/fr3.urdf", "config/fr3.srdf", "assets/asset_path.txt",
]
paths = [ROOT / name for name in NAMES]
paths += [
    path for path in (ROOT / "assets/family_formal").iterdir()
...
destination = ROOT / "FAMILY_FORMAL_SEAL.json"
if destination.exists():
    raise RuntimeError("refuse to overwrite FAMILY_FORMAL_SEAL.json")
